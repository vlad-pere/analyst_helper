...
const generateHtmlContent = (useCase) => {
    ...
    if (useCase.mainScenario.length > 0) {
        ...
        useCase.mainScenario.forEach((step, index) => {
            ...
            const branchingHtml = branchingScenarios.length > 0 ? `<div style="font-style: italic; color: #5e6c84; font-size: 0.9em; margin-top: 5px;">${branchingScenarios.map(sc => `→ См. Альтернативный сценарий: "${sc.name}"`).join('<br>')}</div>` : '';
    ...
    return `<table border="1" style="border-collapse: collapse; width: 100%; font-family: sans-serif;">${tableRows.join('')}</table>`;
};
...
const generateMarkdownContent = (useCase) => {
    ...
    useCase.alternativeScenarios.forEach((sc, index) => {
        ...
        if (sc.startsAtStepId) lines.push(`| *Начинается после* | ${getStepDescription(useCase, sc.startsAtStepId)} |`);
        const mainStepIndex = useCase.mainScenario.findIndex(step => step.id === sc.startsAtStepId);
...
const generateDocxForSingleUseCase = (useCase) => {
    const createCell = (text, isHeader = false, isStepNumber = false) => new TableCell({ children: [new Paragraph({ children: [new TextRun({ text, bold: isHeader })] })], shading: isHeader ? { fill: "FAFAFA", type: ShadingType.CLEAR, color: "auto" } : undefined, width: isStepNumber ? { size: 8, type: WidthType.PERCENTAGE } : undefined });
    const createHeaderRow = (text) => new TableRow({ children: [new TableCell({ children: [new Paragraph({ children: [new TextRun({ text, bold: true })] })], columnSpan: 2, shading: { fill: "F0F2F5", type: ShadingType.CLEAR, color: "auto" } })] });
    const tableRows = [new TableRow({ children: [createCell('Назначение сценария (Цель)', true), createCell(useCase.purpose)] })];
    ...
    if (useCase.mainScenario.length > 0) {
        ...
        useCase.mainScenario.forEach((step, index) => {
            const branchingScenarios = useCase.alternativeScenarios.filter(sc => sc.startsAtStepId === step.id);
            const branchingRuns = branchingScenarios.flatMap(sc => [new TextRun({ text: `→ См. Альтернативный сценарий: "${sc.name}"`, italics: true, break: 1 })]);
            tableRows.push(new TableRow({ children: [createCell(`${index + 1}.`, false, true), new TableCell({ children: [new Paragraph({ children: [new TextRun(step.text || ' '), ...branchingRuns] })] })] }));
        });
    }
    useCase.alternativeScenarios.forEach((sc, index) => {
        ...
        if (sc.returnsToStepId) {
            const returnText = sc.returnsToStepId === 'ends' ? '→ Сценарий завершается' : `→ Возврат к Основному сценарию (${getStepDescription(useCase, sc.returnsToStepId)})`;
            tableRows.push(new TableRow({ children: [new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: returnText, bold: true, italics: true })] })], columnSpan: 2 })] }));
        }
    });
...
export const exportService = {
    copyHtmlToClipboard: async (useCase) => {
        const htmlContent = `<html><head><meta charset='UTF-8'></head><body>${generateHtmlContent(useCase)}</body></html>`;
    ...
    downloadDocx: async (useCase) => {
        const doc = new Document({ sections: [{ children: generateDocxForSingleUseCase(useCase) }] });
        const blob = await Packer.toBlob(doc);
        downloadFile(blob, `${sanitizeFilename(useCase.purpose)}.docx`);
    },
    exportMultipleUseCases: async (useCases, format, filename) => {
        const finalFilename = sanitizeFilename(filename) || 'use-case-report';

        if (format === 'docx') {
            const docChildren = useCases.flatMap((uc, index) => {
                const ucContent = generateDocxForSingleUseCase(uc);
                if (index < useCases.length - 1) {
                    return [...ucContent, new Paragraph({ pageBreakBefore: true })];
                }
                return ucContent;
            });
        ...
        if (format === 'html') {
            const allHtml = useCases.map(uc => `<h1>Use Case: ${uc.purpose || ''}</h1>${generateHtmlContent(uc)}`).join('<hr style="margin: 40px 0; border: 1px solid #ccc;">');
            const fullHtml = `<html><head><meta charset='UTF-8'><title>${filename}</title></head><body>${allHtml}</body></html>`;
            const blob = new Blob([fullHtml], { type: 'text/html' });
            downloadFile(blob, `${finalFilename}.html`);
        }

        if (format === 'md') {
            const allMd = useCases.map(uc => `# Use Case: ${uc.purpose || ''}\n\n${generateMarkdownContent(uc)}`).join('\n\n---\n\n');
            const blob = new Blob([allMd], { type: 'text/markdown' });
            downloadFile(blob, `${finalFilename}.md`);
        }
    }
};